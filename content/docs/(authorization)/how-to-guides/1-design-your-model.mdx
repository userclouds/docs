...
Next, define your <Glossary>edge type</Glossary>s. Each edge type represents a possible one-way relationship two real-world objects can share. They represent a bundle of <Glossary>attribute</Glossary>s (pieces of permission logic) going from a source object type to a target object type. As such, edge types define:
...
Attributes are defined at the edge type level, so all edges of a given type share the same attributes. Each <Glossary>attribute</Glossary> typically achieves one of the following goals:
