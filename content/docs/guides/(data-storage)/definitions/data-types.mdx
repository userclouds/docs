...
Each data type is associated with a Concrete Type, which specifies the actual data structure of the type. The options are `boolean`, `composite`, `date`,`int`,`string`,`timestamp` and `uuid`. Composite data types are complex types that consist of one or more fields, each with its own data type and set of attributes. Composite types allow for the creation of structured, multi-attribute data types, enabling the storage and management of complex data sets within a single column. For a full list of attributes of fields, see our API Reference on [Creating a Data Type](/docs/reference/userstore/config/datatypes/post).
