...
Column defaults are policies that apply to every <<glossary:accessor>> extracting data from a specific column. Column defaults do not apply to mutators. There are three key default settings:
...
- **Default token access policies**: Determine who can access the data in the column via <<glossary:token>> resolution, such as role checks or system identity checks.
